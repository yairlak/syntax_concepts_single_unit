...
class DataHandler:
    ...
    def load_metadata(self, verbose=False):
        if verbose:
            print('Loading metadata...')
        path2mne = os.path.join('..', '..',
                                'data', self.patient[0], 'raw', 'mne')
        
        self.metadata = pd.read_csv(os.path.join(path2mne, 'metadata.csv'))
...
def create_events_array(metadata, sfreq, verbose=False):
    ...
    # Third column
    block_nums = [1] * len(metadata['event_time'])
    ...
    event_type_names = ['Block'] * len(metadata['event_time'])
...
def identify_recording_system(path2data):
    ...
    elif len(glob.glob(os.path.join(path2data, '..', 'macro', '*.ns*')))>0 or len(glob.glob(os.path.join(path2data, '..', 'micro', '*.mat')))>0:
...
def get_time0_timeend(path2data):
    patient = path2data.split('patient_')[1][:3]
    CSC_folders = glob.glob(os.path.join(path2data, 'G??-*/'))
    recording_system = identify_recording_system(path2data)
    if recording_system == 'Neuralynx':
        ...
        time0, timeend = 0, None
    
    return time0, timeend
